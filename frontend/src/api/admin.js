...
export const getAdminStats = async () => {
  try {
    ...
    throw new Error(error.message || '获取统计数据失败，请检查网络连接')
  }
}
...
export const getRecentDiaries = async (limit = 5) => {
  try {
    ...
    throw new Error(error.message || '获取最近日记失败，请检查网络连接')
  }
}
...
export const getDiaryById = async (id) => {
  try {
    ...
    throw new Error(error.message || '获取日记详情失败，请检查网络连接')
  }
}
...
export const getChatRecordById = async (id) => {
  try {
    const response = await api.get(`/admin/chat-records/${id}`)
    if (response.data.success) {
      return response.data.data
    } else {
      throw new Error(response.data.message || '获取聊天记录详情失败')
    }
  } catch (error) {
    console.error('获取聊天记录详情失败:', error.message)
    throw new Error(error.message || '获取聊天记录详情失败，请检查网络连接')
  }
}
...
export const getLettersWithPagination = async (page = 1, size = 10) => {
  try {
    ...
    throw new Error(error.message || '获取信件列表失败，请检查网络连接')
  }
}
