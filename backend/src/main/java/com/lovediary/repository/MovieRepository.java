...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    @Query("SELECT m FROM Movie m WHERE m.user.id = ?1 OR m.partner.id = ?1 OR m.user.partnerId = ?1 OR m.isPublic = true ORDER BY m.createdAt DESC")
    ...
    // 根据标题搜索用户可以查看的电影
    @Query("SELECT m FROM Movie m WHERE (m.user.id = ?1 OR m.partner.id = ?1 OR m.user.partnerId = ?1 OR m.isPublic = true) AND m.title LIKE %?2% ORDER BY m.createdAt DESC")
